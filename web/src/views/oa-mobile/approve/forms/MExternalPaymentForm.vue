...
const formData = reactive({
  paymentReason: '',
  totalAmount: 0,
  requestRefId: null as number | null,
  payeeName: '',
  payeeAccount: '',
  payeeBank: '',
  paymentMethod: '',
...
watch(() => props.data, (newData) => {
  if (newData) {
    formData.paymentReason = newData.paymentReason || newData.reason || ''
    formData.requestRefId = newData.requestRefId || null
    ...
    formData.paymentMethod = newData.paymentMethod || newData.paymentType || ''
    formData.expectedPayDate = newData.expectedPayDate || ''
    formData.contractNo = newData.contractNo || ''
    formData.invoiceNo = newData.invoiceNo || ''
...
watch(formData, () => {
  if (!props.readonly) {
    emit('update:data', {
      ...
      requestRefId: formData.requestRefId,
...
defineExpose({
  ...
  validate: () => {
    if (!formData.paymentReason) return '请输入付款事由'
    if (formData.details.length === 0) return '请添加至少一条付款明细'
    for (const d of formData.details) {
      if (!d.amount || d.amount <= 0) return '付款金额必须大于0'
    }
    return null
  }
})
...
<template>
  <div class="external-payment-form">
    <!-- 付款总额卡片 -->
    ...
    <div class="section-card">
      <div class="field-block no-border">
        ...
        <div v-else class="field-value">{{ formData.paymentReason || '-' }}</div>
      </div>
    </div>
    ...
    <div class="section-card" v-for="(item, index) in formData.details" :key="item._uid || index">
      <div class="detail-tab">
        <span class="detail-tab-text">付款明细 {{ index + 1 }}</span>
      ...
      <div class="field-block no-border">
        ...
        <div v-else class="field-value">{{ item.remark || '-' }}</div>
      </div>
    </div>
    ...
    <div class="section-card">
      <div class="field-block">
        ...
        <div v-else class="field-value">{{ formData.payeeName || '-' }}</div>
      </div>
      ...
      <div class="field-block">
        ...
        <div v-else class="field-value">{{ formData.payeeAccount || '-' }}</div>
      </div>
      ...
      <div class="field-block">
        ...
        <div v-else class="field-value">{{ formData.payeeBank || '-' }}</div>
      </div>
      ...
      <div class="field-block">
        ...
        <div v-else class="field-value">{{ formData.paymentMethod || '-' }}</div>
      </div>
      ...
      <div class="field-block">
        ...
        <div v-else class="field-value">{{ formatDate(formData.expectedPayDate) }}</div>
      </div>
      ...
      <div class="field-block">
        ...
        <div v-else class="field-value">{{ formData.contractNo || '-' }}</div>
      </div>

      <div class="field-block no-border">
        <div class="field-label">发票号</div>
        <template v-if="!props.readonly">
          <VanField
        ...
        <div v-else class="field-value">{{ formData.invoiceNo || '-' }}</div>
      </div>
    </div>
