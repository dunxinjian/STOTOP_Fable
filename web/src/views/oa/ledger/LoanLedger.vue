...
import { getLoanLedger } from '@/api/oa'
...
const columns: TableColumnsType = [
  ...
  { title: '借款金额', dataIndex: 'totalLoaned', key: 'totalLoaned', width: 140, align: 'right' },
  { title: '已冲抵', dataIndex: 'totalDeducted', key: 'totalDeducted', width: 140, align: 'right' },
  { title: '已还款', dataIndex: 'totalRepaid', key: 'totalRepaid', width: 140, align: 'right' },
...
async function loadData() {
  loading.value = true
  try {
    const res = await getLoanLedger(searchParams) as any
  ...
  <div class="page-container">
    ...
    >
      <template #bodyCell="{ column, record }">
        <template v-if="column.key === 'totalLoaned'">{{ formatAmount(record.totalLoaned) }}</template>
