...
public class LoanApplyService : ILoanApplyService
{
    ...
    public LoanApplyService(STOTOPDbContext db)
    {
        _db = db;
    }

    public async Task<PagedResult<LoanApplyDto>> GetPagedListAsync(long userId, int page, int pageSize, int? status, long? orgId)
    {
        var query = _db.Set<OaLoanApplication>().AsNoTracking().AsQueryable();
        ...
        var result = items.Select(e => new LoanApplyDto
        {
            Id = e.FID,
            ...
            LoanAmount = e.FLoanAmount,
            LoanReason = e.FLoanReason,
            ...
            ReimburseOffsetAmount = e.FReimburseOffsetAmount,
        ...
        return new PagedResult<LoanApplyDto>
        {
            Items = result,
            Total = total,
            PageIndex = page,
    ...
    public async Task<LoanApplyDto?> GetByIdAsync(long id)
    {
        var entity = await _db.Set<OaLoanApplication>().AsNoTracking().FirstOrDefaultAsync(e => e.FID == id);
        if (entity == null)
            return null;
        ...
        return new LoanApplyDto
        {
            ...
            OrgId = entity.FOrgId,
            OrgName = org?.FName ?? "",
            LoanAmount = entity.FLoanAmount,
            LoanReason = entity.FLoanReason,
            ...
            ReimburseOffsetAmount = entity.FReimburseOffsetAmount,
    ...
    public async Task<LoanApplyDto> CreateAsync(CreateLoanApplyRequest request, long userId)
    {
        // 生成单据编号：JKSQ + yyyyMMdd + 3位序号
        var docNumber = await GenerateDocNumberAsync("JKSQ");

        var entity = new OaLoanApplication
        {
            FDocNumber = docNumber,
            FApplicantId = userId,
            FDeptId = request.DeptId,
            FOrgId = request.OrgId,
            FLoanAmount = request.LoanAmount,
            FLoanReason = request.LoanReason,
            ...
            FDocStatus = 0, // 草稿
            FReimburseOffsetAmount = 0,
            FRepaidAmount = 0,
            FOutstandingBalance = request.LoanAmount,
            FCreatedTime = DateTime.Now
        };

        _db.Set<OaLoanApplication>().Add(entity);
        await _db.SaveChangesAsync();

        return (await GetByIdAsync(entity.FID))!;
    }

    public async Task<LoanApplyDto?> UpdateAsync(long id, UpdateLoanApplyRequest request, long userId)
    {
        var entity = await _db.Set<OaLoanApplication>()
            .AsTracking()
        ...
        entity.FLoanAmount = request.LoanAmount;
        entity.FLoanReason = request.LoanReason;
        ...
        entity.FOutstandingBalance = request.LoanAmount - entity.FReimburseOffsetAmount - entity.FRepaidAmount;
    ...
    public async Task<bool> DeleteAsync(long id, long userId)
    {
        ...
        _db.Set<OaLoanApplication>().Remove(entity);
        await _db.SaveChangesAsync();
    ...
    public async Task SubmitAsync(long id, long userId)
    {
        ...
        if (entity == null)
            throw new InvalidOperationException("借款申请单不存在");
        ...
        throw new NotSupportedException("BPM流程已废除，请通过 CardFlow 发起借款审批。");
    }

    public async Task<List<LoanLedgerDto>> GetLedgerAsync(long? orgId, long? applicantId)
    {
        var query = _db.Set<OaLoanApplication>().AsNoTracking().AsQueryable();
        if (orgId.HasValue)
            query = query.Where(e => e.FOrgId == orgId.Value);
        ...
        var loans = await query.OrderByDescending(e => e.FCreatedTime).ToListAsync();
        // 批量获取申请人姓名
        var applicantIds = loans.Select(e => e.FApplicantId).Distinct().ToList();
        var users = await _db.Set<SysUser>()
            .AsNoTracking()
            .Where(u => applicantIds.Contains(u.FID))
            .ToDictionaryAsync(u => u.FID, u => u.FName);

        return loans.Select(e => new LoanLedgerDto
        {
            Id = e.FID,
            DocNumber = e.FDocNumber,
    ...
    private async Task<string> GenerateDocNumberAsync(string prefix)
    {
        ...
        // 查询当天最大编号
        var maxNumber = await _db.Set<OaLoanApplication>()
            .AsNoTracking()
