...
public class StgShentongFakeSignComplaintConfiguration : IEntityTypeConfiguration<StgShentongFakeSignComplaint>
{
    public void Configure(EntityTypeBuilder<StgShentongFakeSignComplaint> builder)
    {
        builder.ToTable("STG申通_虚签投诉明细");
        ...
        builder.Property(e => e.F省份编码).HasColumnName("F省份编码").HasMaxLength(200);
        ...
        builder.Property(e => e.F预估考核金额).HasColumnName("F预估考核金额").HasMaxLength(200);
        ...
        builder.HasIndex(e => e.F批次ID).HasDatabaseName("IX_STG申通_虚签投诉明细_F批次ID");
        builder.HasIndex(e => e.FDataScopeId).HasDatabaseName("IX_STG申通_虚签投诉明细_数据作用域").HasFilter("[FDataScopeId] IS NOT NULL");

        // 跨批次去重唯一索引（实际由 Seeder DDL 创建，此处声明保持模型一致性）
        // 虚签投诉明细每工单一行，按工单号去重；1 字段以兼容 ExcelInputPlugin。
        builder.HasIndex(e => new { e.F工单号, e.FOrgId })
            .IsUnique()
            .HasFilter("[FIsRevoked] = 0 AND [F工单号] IS NOT NULL AND [F工单号] != ''")
            .HasDatabaseName("UX_STG申通_虚签投诉明细_工单号_未撤销");
    }
}
