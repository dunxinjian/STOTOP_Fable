...
public class StgShentongInfoIndexCompleteConfiguration : IEntityTypeConfiguration<StgShentongInfoIndexComplete>
{
    public void Configure(EntityTypeBuilder<StgShentongInfoIndexComplete> builder)
    {
        builder.ToTable("STG申通_物流信息完整汇总");
        ...
        builder.Property(e => e.F揽收缺失量).HasColumnName("F揽收缺失量").HasMaxLength(200);
        builder.Property(e => e.F揽收缺失率).HasColumnName("F揽收缺失率").HasMaxLength(200);
        builder.Property(e => e.F签收总量).HasColumnName("F签收总量").HasMaxLength(200);
        builder.Property(e => e.F派件缺失量).HasColumnName("F派件缺失量").HasMaxLength(200);
        builder.Property(e => e.F派件缺失率).HasColumnName("F派件缺失率").HasMaxLength(200);
        builder.Property(e => e.F到件缺失量).HasColumnName("F到件缺失量").HasMaxLength(200);
        builder.Property(e => e.F到件缺失率).HasColumnName("F到件缺失率").HasMaxLength(200);
        ...
        builder.HasIndex(e => e.F批次ID).HasDatabaseName("IX_STG申通_物流信息完整汇总_F批次ID");
        builder.HasIndex(e => e.FDataScopeId).HasDatabaseName("IX_STG申通_物流信息完整汇总_数据作用域").HasFilter("[FDataScopeId] IS NOT NULL");

        // 跨批次去重唯一索引（实际由 Seeder DDL 创建，此处声明保持模型一致性）
        // 物流信息完整汇总 1 行/网点/统计日期；本文件无网点编号，按「网点名称 + 统计日期」去重；2 字段以兼容 ExcelInputPlugin。
        builder.HasIndex(e => new { e.F网点名称, e.F统计日期, e.FOrgId })
            .IsUnique()
            .HasFilter("[FIsRevoked] = 0 AND [F网点名称] IS NOT NULL AND [F网点名称] != ''")
            .HasDatabaseName("UX_STG申通_物流信息完整汇总_网点日期_未撤销");
    }
}
